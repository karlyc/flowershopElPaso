// js/countries.js — dial codes for the phone country-code selector.
// USA and Mexico are pinned to the top (El Paso border-city customer base);
// the rest are available in the same dropdown under "All countries".
...
// Renders <optgroup>s: common countries first, then the full list, with `selected` pre-picked.
// Several countries share a dial code (e.g. USA/Canada both use +1) — only the first match
// (from Common) is marked selected, so the picker doesn't silently default to the wrong one.
export function countryCodeOptions(selected = '+1') {
  let matched = false;
  const opt = (c) => {
    const isSelected = !matched && c.dial === selected;
    if (isSelected) matched = true;
    return `<option value="${c.dial}" ${isSelected ? 'selected' : ''}>${c.name} (${c.dial})</option>`;
  };
  return `
    <optgroup label="Common">${COMMON_COUNTRIES.map(opt).join('')}</optgroup>
    <optgroup label="All countries">${ALL_COUNTRIES.map(opt).join('')}</optgroup>
  `;
}
